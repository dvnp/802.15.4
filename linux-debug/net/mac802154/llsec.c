		 ...
		 struct ieee802154_llsec_key_id *key_id)
{
	...
	pr_debug("%s\t key_id_mode=%d devaddr.mode=%d\n", __func__, key_id_mode, devaddr.mode);
	...
	list_for_each_entry_rcu(key_entry, &sec->table.keys, list) {
		...
		pr_debug("\t frame_types=%#04x type=%#04x (BIT %#04lx)\n", key_entry->key->frame_types, hdr->fc.type, BIT(hdr->fc.type));
		if (!(key_entry->key->frame_types & BIT(hdr->fc.type)))
			continue;

		pr_debug("\t mode=%d == key_id_mode=%d\n", id->mode, key_id_mode);
		...
		if (key_id_mode == IEEE802154_SCF_KEY_IMPLICIT) {
			...
			pr_debug("\t id=%d == key_id=%d\n", id->id, hdr->sec.key_id);
...
static int llsec_do_encrypt(struct sk_buff *skb,
			    ...
			    struct mac802154_llsec_key *key)
{
	__le64 *Lkey = (__le64*)key->key.key,
				  *Hkey = (__le64*)&key->key.key[8];

	pr_debug("%s:\n\t sec->params.\n\t  "
			"enabled=%d\n\t  "
			"frame_counter=%d\n\t  "
			"out_level=%d\n\t  "
			"default_key_source=%#llx\n\t  "
			"pan_id=%#06x\n\t  "
			"hwaddr=%#llx\n\t  "
			"coord_hwaddr=%#llx\n\t  "
			"coord_shortaddr=%#06x\n\t  "
			...
			__func__,
			sec->params.enabled,
			sec->params.frame_counter,
			sec->params.out_level,
			sec->params.default_key_source,
			sec->params.pan_id,
			sec->params.hwaddr,
			sec->params.coord_hwaddr,
			sec->params.coord_shortaddr,
			sec->params.out_key.mode,
			sec->params.out_key.id);

	pr_debug("\t hdr->sec.level=%d\n\t key->\n\t  "
			"frame_types=%02x\n\t  "
			"cmd_frame_ids=%d\n\t "
			"key=%#llx%llx\n",
			hdr->sec.level,
			key->key.frame_types,
			key->key.cmd_frame_ids,
			*Hkey, *Lkey);
...
int mac802154_llsec_encrypt(struct mac802154_llsec *sec, struct sk_buff *skb)
{
	...
	pr_debug("%s HEADER enabled=%d version=%d sec.level=%d\n", __func__, hdr.fc.security_enabled, hdr.fc.version, hdr.sec.level);
	...
	pr_debug("%s DATA_SIZE=%d (len=%d hlen=%d authlen=%d)  < MTU=%d \n",
			__func__, skb->len + hlen + authlen + IEEE802154_MFR_SIZE, skb->len, hlen, authlen, IEEE802154_MTU);
